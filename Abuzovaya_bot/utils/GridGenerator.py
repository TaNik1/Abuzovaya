...
class GridGenerator:
    def __init__(self, star_count):
        ...
        # Load images
        self.background = Image.open(os.path.join("/home/nikitat612006/d/Abuzovaya/Abuzovaya_bot/src/image/fragments", 'background.png')).convert("RGBA")
        self.frame = Image.open(os.path.join("/home/nikitat612006/d/Abuzovaya/Abuzovaya_bot/src/image/fragments", 'frame.png')).convert("RGBA")
        ...
        for i in range(self.grid_size):
            for j in range(self.grid_size):
                ...
                if (i, j) in star_positions:
                    star = Image.open(os.path.join("/home/nikitat612006/d/Abuzovaya/Abuzovaya_bot/src/image/fragments",
                                                   f'звезды_{0 if i * 5 + j + 1 < 10 else ""}{i * 5 + j + 1}.jpg')).convert(
                        "RGBA")
                    grid_img.paste(star, (x, y), star)  # No mask needed for star
                else:
                    square = Image.open(os.path.join("/home/nikitat612006/d/Abuzovaya/Abuzovaya_bot/src/image/fragments",
